...
# ([^-]+) 代表没有遇到小横杠-就一直匹配下去
ret = re.match('([^-]+)-(\d+)', 'abc123-66668888')
if ret:
    print(ret.group())
    print(ret.group(1))
    print(ret.group(2))
...
# \：引用分组
ret = re.match('<[a-zA-Z]*>\w*</[a-zA-Z]*>', '<html>yf</html>')
print(ret.group())

ret = re.match(r'<([a-zA-Z]*)>\w*</\1>', '<html>yf</html>')
print(ret.group())
...
# 给分组起别名，通过别名来引用分组
ret = re.match(r'<(?P<n1>\w*)><(?P<n2>\w*).*</(?P=n2)></(?P=n1)>', '<html><h1>www.cskaoyan.com</h1></html>')
print(ret.group())

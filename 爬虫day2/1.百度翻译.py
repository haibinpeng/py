...
class SignAndToken:
    ...
    def __init__(self):
        ...
        self.session.headers = self.headers  # session.headers
        # 请求可以拿到一个token
        response = self.session.get("https://fanyi.baidu.com/")  # session.get
        # 获取token
        ...
        # 获取gtk
        # 获取window.gtk的值
        self.gtk = re.findall(r";window.gtk = (.*?);", response.content.decode())[0]
...
tool = TranslateTool()
response = json.loads(tool.translate("hello").decode())
# print(response)
